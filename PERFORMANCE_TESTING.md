...
Kerry-Lynn Whyte’s sketch measurements from **2026-08-31** (HTTP) and **2026-09-01** (Redis enqueue + five live Testnet settlements) remain in `backend/perf/results/run1_*.csv` for comparison. Live XRPL settlement was **not** repeated on 2026-09-07 (it would spend UCTUSD from the demo treasury); §2 keeps the 12.4 s/tx Testnet sample and labels it as such.
...
**Queue enqueue (re-measured 2026-09-07 on this fork):** 200 `enqueue_settlement` calls (SQLite insert + commit, Redis `XADD`, then a second commit storing `stream_entry_id`). Entries were removed from the stream afterwards so they could not sit ahead of demo work.

| Metric | Result | When |
|---|---|---|
| Message-queue enqueue throughput (this host) | 200 messages in 8.641 s → **23.1 msg/s** | 2026-09-07, this fork |
...
- Settlement success **5/5** on the 2026-09-01 Testnet sample (`tesSUCCESS`, no retries). Not re-measured against the demo treasury.
...
Optional enqueue-only timing (creates then deletes 200 synthetic remittances; leaves `benchmark-sender@example.com` / `benchmark-recipient@example.com`). Do **not** omit `--enqueue-only` on the demo treasury unless you intend live Testnet Payments. The 12.4 s/tx figures in §2 are the 2026-09-01 sample.
