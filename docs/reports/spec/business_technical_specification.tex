...
\section{Database}
Runtime schema is Alembic, not \texttt{create\_all} alone. FastAPI lifespan runs \texttt{alembic upgrade head} then seeds default fees and limit tiers. Tests still use in-memory \texttt{create\_all}. Figure~\ref{fig:er} is the entity--relationship view; the list below is the column notes.
...
\begin{tikzpicture}[
  font=\scriptsize,
  x=2.42cm, y=1.22cm,
  ent/.style={draw, rounded corners=1pt, align=center, inner sep=2.4pt, minimum height=0.70cm, minimum width=2.22cm},
  arr/.style={-{Latex}, semithick},
  lab/.style={font=\tiny, inner sep=1pt, fill=white}
]
\node[ent] (sess) at (0,3.2) {sessions};
\node[ent] (kyc) at (2,3.2) {kyc\_applications};
\node[ent] (co) at (4,3.2) {cash\_out\_requests};
\node[ent] (users) at (2,2.1) {users};
\node[ent] (wal) at (4,2.1) {recipient\_wallets};
\node[ent] (ben) at (0,1.0) {beneficiaries};
\node[ent] (rem) at (2,1.0) {remittances};
\node[ent] (set) at (2,-0.1) {settlement\_messages};
\node[ent] (fee) at (0,-1.2) {fee\_config};
\node[ent] (lim) at (2,-1.2) {limit\_tiers};
\node[ent] (pw) at (4,-1.2) {platform\_wallet};
\draw[arr] (sess) -- node[lab, left] {N:1} (users);
\draw[arr] (kyc) -- node[lab, right] {0..1:1} (users);
\draw[arr] (co) -- node[lab, right] {N:1} (users);
\draw[arr] (wal) -- node[lab, above] {0..1:1} (users);
\draw[arr] (ben) -- node[lab, left] {N:1 sender} (users);
\draw[arr, dashed] (ben.north) to[bend left=18] node[lab, above left] {0..1 linked} (users.west);
\draw[arr] (rem) -- node[lab, right] {N:1 sender} (users);
\draw[arr] (rem) -- node[lab, above] {N:1} (ben);
\draw[arr] (set) -- node[lab, right] {1:1} (rem);
\end{tikzpicture}
\caption{Entity--relationship view of the Alembic schema. Cardinalities are child:parent. The dashed edge is optional \texttt{linked\_user\_id}. Cash-out is not foreign-keyed to a remittance: payouts draw aggregated spendable UCTUSD. \texttt{fee\_config} and \texttt{platform\_wallet} are singletons; \texttt{limit\_tiers} is the unverified/verified lookup.}
\label{fig:er}
\end{figure}
...
\label{sec:perf}
HTTP load, queue enqueue, and five live Testnet Payments were re-run on this fork on 7~September 2026 (50 concurrent synthetic users, 60~s, Windows, single-process uvicorn and SQLite). The tables and charts live in \texttt{PERFORMANCE\_TESTING.md} and \texttt{backend/perf/results/}.

The HTTP mix delivered 2{,}218 requests, zero failures, about 37~req/s. Authenticated reads sat around 20--33~ms at the median; \texttt{POST /remittances} about 66~ms median. \texttt{POST /auth/login} remains the outlier (about 850~ms median) because bcrypt is deliberately expensive. Queue enqueue on this OneDrive-backed SQLite host measured about 29 messages/s (a 1~September Mac sample of the same Redis path was 517~msg/s). Five real Testnet Payments completed 5/5 and averaged about 17.5~s each---ledger close time, not application CPU (a 1~September sample of the same script was 12.4~s/tx). Relative bottlenecks---bcrypt on login, consensus on settlement---are unchanged in kind.
