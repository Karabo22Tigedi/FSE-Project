...
The journey follows the brief \citep{UCTBrief2026} and is implemented end-to-end on the API (the web UI in Section~\ref{sec:ui} is planned, not yet built).
...
\begin{enumerate}
  ...
  \item The recipient opens \texttt{GET /wallet/me}, sees spendable RLUSD, incoming transfers with hashes, and cash-out history.
  \item The recipient requests a simulated cash-out to USD or ZAR. An admin approves, completes, or fails it. Failed cash-outs refund spendable balance. There is no reverse on-chain burn.
\end{enumerate}

Anyone with a tracking reference who is the sender, the linked recipient, or an admin can call \texttt{GET /remittances/track/\{ref\}}.
...
\label{sec:arch}
Figure~\ref{fig:arch} is the runtime. FastAPI serves JSON. SQLAlchemy talks to SQLite through Alembic revisions \texttt{0001\_initial} and \texttt{0002\_quote\_session}. Redis is transport for settlement, not the source of truth. The settlement worker (the same function as \texttt{POST /admin/settlement/run} and \texttt{python -m scripts.run\_settlement\_worker}) signs Testnet Payments with \texttt{xrpl-py}. CORS is enabled for local UI origins (Vite 5173, React 3000, and the API host).
...
\node[box, minimum width=2.6cm] (ui) {Planned web UI\\(localhost CORS)};
\node[box, minimum width=2.8cm, right=0.55cm of ui] (api) {FastAPI REST\\OpenAPI \texttt{/docs}};
\node[box, minimum width=2.4cm, right=0.55cm of api] (db) {SQLite\\Alembic 0001+0002};
...
\draw[arr] (wal) -- (co);
\end{tikzpicture}
\caption{Settlement path: confirmed rand in, queued Payment, custodial RLUSD, simulated fiat out.}
\label{fig:flow}
\end{figure}
...
\section{Cash-in, RLUSD settlement, and cash-out}
...
\subsection{Cash-out}
The recipient must have approved KYC to cash out but may hold RLUSD without it. Spendable \texttt{RecipientWallet.balance} is debited atomically at request time (\texttt{UPDATE \ldots WHERE balance >= :amt}). \texttt{GET /wallet/me} reports \texttt{balance\_rlusd} and \texttt{spendable\_balance} as that spendable figure. \texttt{on\_chain\_balance} is spendable plus cash-out amounts in \texttt{requested}, \texttt{approved}, or \texttt{completed} (failed cash-outs are excluded). Simulated cash-out does not burn tokens on the ledger, so after a request spendable drops while on-chain stays higher by the reserved amount. A failed cash-out refunds spendable. Completion is a status transition, not a second XRPL Payment back to treasury.
...
Consumer protection and safeguarding follow from the same honesty: quoted fees should match charged fees (our quote locks the breakdown for 15 minutes); customer secrets should not leak (Section~11); and simulated balances must not be described as redeemable bank money. We do not hold client fiat in a trust account because we never collect real fiat. A production remittance UI would still need a pre-contract fee disclosure, a receipt with the tracking reference, and a complaints path---none of which a Swagger page substitutes for. The planned screens in Section~\ref{sec:ui} are where those disclosures would live.
...
\begin{itemize}
  ...
  \item Cash-in and cash-out are simulated; there is no reverse on-chain cash-out.
...
\label{sec:perf}
We cite Kerry-Lynn Whyte's Locust and settlement timings as a \textbf{baseline}, not as a re-run of this fork. The numbers were taken on 31~August and 1~September 2026 against a local single-process uvicorn and SQLite, and are recorded in \texttt{PERFORMANCE\_TESTING.md}. They remain useful as an order-of-magnitude picture of the same FastAPI surface.

The HTTP mix (50 concurrent synthetic users, 60~s, 2{,}324 requests, zero failures) delivered about 39~req/s. Read endpoints sat in the high-single-digit milliseconds at the median; \texttt{POST /remittances} about 11~ms median. \texttt{POST /auth/login} was the outlier at about 400~ms median because bcrypt is deliberately expensive. Redis Stream enqueue measured about 517 messages/s for 200 publishes. Five real Testnet Payments averaged about 12.4~s each---ledger close time, not application CPU. We have not repeated Locust on the patched worker (PEL reclaim, outbox, hashed sessions). Relative bottlenecks---bcrypt on login, consensus on settlement---are unchanged in kind.

\section{Planned web UI}
\label{sec:ui}
The backend is API-complete; the brief still requires a web application \citep{UCTBrief2026}. This page is the screen map. The UI may be any SPA that speaks JSON (for example Vue on 5173 or React on 3000). CORS is already open for those origins. No framework is mandated here.

Table~\ref{tab:ui} is the screen map. Empty states matter: unverified senders see KYC pending, not a send form; expired quotes show ``expired'' not a cash-in button; a recipient without a Testnet account yet still sees a zero spendable wallet row.

\begin{table}[ht]
\centering
\caption{Planned screens mapped to existing endpoints (framework-agnostic).}
\label{tab:ui}
\footnotesize
\begin{tabular}{@{}p{0.22\textwidth}p{0.34\textwidth}p{0.36\textwidth}@{}}
\toprule
\textbf{Screen} & \textbf{What the user sees} & \textbf{Endpoints} \\
\midrule
Register / login &
Name, email, mobile, password; logout &
\texttt{POST /auth/register}, \texttt{/login}, \texttt{/logout} \\
Sender home &
Profile, KYC badge, remaining daily/monthly ZAR &
\texttt{GET /users/me}, \texttt{/kyc/me/status}, \texttt{/limits/me} \\
KYC form &
Brief fields; resubmit if rejected &
\texttt{POST /kyc}, \texttt{GET /kyc/me} \\
Beneficiaries &
List and add; linked vs unlinked &
\texttt{GET}/\texttt{POST /beneficiaries} \\
New quote &
ZAR, beneficiary, locked fee/FX/RLUSD, 15~min timer, \texttt{MG} ref &
\texttt{POST /remittances} \\
Quote actions &
Cancel or choose cash-in method &
\texttt{POST \ldots/cancel}, \texttt{\ldots/cash-in} \\
Track / history &
Status, hash once settled &
\texttt{GET /remittances/me}, \texttt{/remittances/track/\{ref\}} \\
Recipient wallet &
Spendable vs on-chain, address, inbound list, cash-outs &
\texttt{GET /wallet/me} \\
Cash-out &
USD or ZAR amount; pending statuses &
\texttt{POST /cash-outs}, \texttt{GET /cash-outs/me} \\
Admin KYC &
Queue, approve, reject with reason &
\texttt{GET /kyc}, \texttt{POST \ldots/approve\textbar reject} \\
Admin cash-in &
Pending methods; confirm receipt &
\texttt{GET /remittances}, \texttt{POST \ldots/confirm-cash-in} \\
Admin settlement &
Stream ids, retry stuck rows, run worker &
\texttt{GET /admin/settlement}, \texttt{POST \ldots/run}, \texttt{\ldots/\{id\}/retry} \\
Admin cash-out &
Approve, complete, or fail (refund) &
\texttt{GET /cash-outs}, \texttt{POST \ldots/approve\textbar complete\textbar fail} \\
Admin config &
Fees and limit tiers &
\texttt{GET}/\texttt{PUT /admin/fee-config}, \texttt{/admin/limit-tiers} \\
\bottomrule
\end{tabular}
\end{table}

Admins are created with \texttt{scripts/create\_admin.py}, never via public register. Demonstration until the UI exists: Swagger at \texttt{/docs} plus the settlement worker script. That is an API demo, not the marked web application.
