  ...
  useEffect(() => {
    ...
    return startThreeLoop(canvas, {
      ...
      setup: ({ scene }) => {
        ...
        phone.position.set(1.15, -0.55, 0.1)
        phone.rotation.set(-1.12, 0.18, 0.38)
        scene.add(phone)
        ...
        const coins = [
          { color: 0xd9dee6, pos: [-1.55, 0.85, 0.6] as const, spin: 0.7 },
          { color: 0x2c71f6, pos: [0.15, 0.25, 1.3] as const, spin: -0.55 },
          { color: 0x3de0c4, pos: [2.05, 0.95, -0.2] as const, spin: 0.9 },
          { color: 0xe8c872, pos: [-0.85, -0.05, -0.7] as const, spin: -0.4 },
        ].map((item) => {
        ...
        return {
          update: (t) => {
            const drift = isFooter ? t * 0.22 : t * 0.28
            phone.rotation.z = 0.38 + Math.sin(drift * 0.7) * 0.06
            phone.position.y = -0.55 + Math.sin(drift) * 0.08

            craft.position.x = Math.sin(drift * 0.85) * 1.35 + (isFooter ? 0.4 : -0.15)
            craft.position.y = 1.25 + Math.sin(drift * 1.6) * 0.22
            craft.position.z = Math.cos(drift * 0.7) * 0.85
            ...
            for (const item of coins) {
              ...
              item.coin.position.y = item.origin[1] + Math.sin(t * 0.9 + item.spin) * 0.16
            }

            for (const item of packets) {
              const u = t * 0.55 + item.offset
              item.packet.position.set(Math.sin(u) * 2.4, 0.4 + Math.cos(u * 1.3) * 0.5, Math.cos(u * 0.8) * 1.4)
